<template>
  <div class="employee-wrapper">
    <v-data-table
      :headers="headers"
      :items="items.data"
      :options.sync="options"
      :server-items-length="items.total"
      class="elevation-0"
      :item-class="employeeRowClass"
      :loading="isLoading"
      item-key="id"
    >
      <template v-slot:top>
        <div class="employee-wrapper-titleslot">
          <h1>{{ title }}</h1>
          <v-spacer></v-spacer>
          <v-btn tile class="mr-6 add-employee-btn" @click="NewItem">
            <v-icon left> mdi-plus-box </v-icon>
            New {{ title }}
          </v-btn>
          <v-text-field
            v-model.lazy="search"
            label="Search"
            class="mx-4"
            append-icon="mdi-magnify"
            @change="searchFunction"
          ></v-text-field>
        </div>
      </template>
      <template v-slot:[`item.avatar`]="{ item }">
        <div class="p-2">
          <v-avatar size="36">
            <v-img :src="item.avatar"> </v-img>
          </v-avatar>
        </div>
      </template>
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon
          small
          v-if="PermissionAsign"
          class="mr-2"
          @click="AssignPermission(item)"
        >
          mdi-lock
        </v-icon>
        <v-icon small class="mr-2" @click="UpdateItem(item)">
          mdi-pencil
        </v-icon>
        <v-icon v-if="item.status != `Deleted`" small @click="RemoveItem(item)">
          mdi-delete
        </v-icon>
      </template>
    </v-data-table>
  </div>
</template>

<script>
export default {
  props: {
    headers: {
      type: Array
    },
    items: {
      type: Object
    },
    hasSearch: {
      type: Boolean,
      default: false
    },
    reload: {
      type: Function
    },
    title: {
      type: String
    },
    hasAction: {
      type: Boolean,
      default: true
    },
    viewPermission: {
      type: Boolean,
      default: false
    },
    PermissionAsign: {
      type: Boolean,
      default: false
    }
  },
  data() {
    return {
      isLoading: false,
      options: {},
      search: "",
      searched: [],
      AwaitingSearch: false
    };
  },
  created() {},
  computed: {
    /*params() {
      return {
        ...this.options,
        search: this.search
      }
    }*/
  },
  watch: {
    options: {
      handler(val) {
        const { search } = this.$route.query;
        const query = {
          ...this.mapQuery(val),
          search: search ?? ""
        };
        this.loadData(query);
      },
      deep: true
    },
    $route: {
      handler(to) {
        if (this.hasSearch) {
          const { search } = to.query;
          const query = {
            ...this.mapQuery(this.options),
            search
          };
          this.loadData(query);
        }
      }
    },
    params: {
      handler(to) {
        if (this.hasSearch) {
          const search = to.search;
          const query = {
            ...this.mapQuery(this.options),
            search
          };
          this.loadData(query);
        }
      }
    }
  },
  methods: {
    AssignPermission(item) {
      this.$emit("AssignPermission", item);
    },
    UpdateItem(item) {
      this.$emit("UpdateItem", item);
    },
    RemoveItem(item) {
      this.$emit("RemoveItem", item);
    },
    NewItem() {
      this.$emit("NewItem");
    },
    loadData(query) {
      if (this.viewPermission) {
        this.isLoading = true;
        this.reload(query).then(() => {
          this.isLoading = false;
        });
      }
    },
    mapQuery({ page, itemsPerPage, sortBy, sortDesc }) {
      return {
        page,
        limit: itemsPerPage,
        sort: sortBy[0] ?? "",
        order: sortDesc[0] ? "desc" : "asc"
      };
    },
    employeeRowClass() {
      return "employees-row";
    },
    searchFunction() {
      if (this.hasSearch) {
        const search = this.search;
        const query = {
          ...this.mapQuery(this.options),
          search
        };
        this.loadData(query);
      }
    }
  }
};
</script>

<style lang="scss">
.employee-row {
  height: 66px;
}
.md-field {
  max-width: 300px;
}
.employee-wrapper {
  padding: 25px;
  background-color: #f7f7e8;

  .employee-wrapper-titleslot {
    background-color: #f7f7e8 !important;

    h1 {
      color: rgba(0, 0, 0, 0.6);
    }

    background-color: #f7f7e8;
    display: flex;
    justify-content: center;
    align-items: center;

    .search {
      padding-top: 0 !important;
    }

    .add-employee-btn {
      background-color: #cfd3ce !important;
      border-radius: 15px;
    }
  }
}

.v-data-table-header {
  background-color: #c4c4c4 !important;
}
.employee-wrapper-title {
  margin-bottom: 25px;
  h1 {
    color: rgba(0, 0, 0, 0.6);
  }
  background-color: #f7f7e8;
  display: flex;
  justify-content: center;
  align-items: center;
  .search {
    padding-top: 0 !important;
  }

  .add-employee-btn {
    background-color: #cfd3ce !important;
    border-radius: 15px;
  }
}
</style>
